...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in details_link:
            if 'catalogue' in link:
                yield scrapy.Request(url=urljoin("https://books.toscrape.com/", link), callback=self.parse_link)
            else:
                yield scrapy.Request(url=urljoin("https://books.toscrape.com/catalogue/", link), callback=self.parse_link)
    ...
    def parse_link(self, response):
        ...
        breadcrumb = response.xpath("//ul[@class = 'breadcrumb']/li//text()").getall()
        indices_to_keep = [1, 4, 7, 9]
        breadcrumb = [breadcrumb[i] for i in indices_to_keep]
        print(breadcrumb)
